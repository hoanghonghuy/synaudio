...
const previewAsset = computed(() => previewAssetForChapter({
  activeChapterID: activeChapter.value?.ID ?? '',
...
const audioStatus = computed(() => {
  if (selectionLoading.value) return 'Đang tải…'
  if (activeAudio.value) return `ACTIVE v${activeAudio.value.VersionNo} · ${activeAudio.value.ID} · checksum ${activeAudio.value.Checksum || '—'} · ${activeAudio.value.DurationMs}ms`
...
const publishMissingLabels: Record<string, string> = {
  approved_content: 'Approved content', approved_content_authority: 'Approved content authority', narration: 'Narration revision',
  active_audio: 'Active durable audio', active_audio_authority: 'Active audio authority', active_audio_stale: 'Active audio khớp narration mới nhất',
  story_status: 'Story status cho phép publish', story_publish_authority: 'Story publish authority', publish_authority: 'Publish authority',
}
const publishStatus = computed(() => {
  ...
  if (activeChapter.value?.Status !== 'READY') {
    if (publishReadiness.value.ready) return `READY gates passed — chapter ${activeChapter.value?.Status || '—'} có thể Mark Ready trước publish`
...
async function loadAudioPreview() {
  ...
  try {
    const response = await getAdminAudioPreviewURL(chapter.ID, asset.ID)
    audioPreview.value = request.succeed(response.url)
  } catch (e) {
...
async function loadAudioProjections(chapterID: string) {
  const [narration, active] = await Promise.all([getLatestNarrationRevision(chapterID), getActiveAudioAsset(chapterID)])
...
async function selectChapter(chapter: Chapter) {
  ...
  try {
    const [revisionResponse, reviewResponse, audioState, readiness] = await Promise.all([
      listContentRevisions(chapter.ID), listChapterReviews(chapter.ID), loadAudioProjections(chapter.ID), getPublishReadiness(chapter.ID),
    ])
...
async function startGeneration() {
  ...
  action.value = 'start-generation'; error.value = ''
  try { const run = await startChapterGeneration(storyID.value, chapter.ID); if (activeChapter.value?.ID === chapter.ID) generationRun.value = run }
  catch (e) { if (activeChapter.value?.ID === chapter.ID) error.value = e instanceof Error ? e.message : 'Không thể bắt đầu Chapter Generation.' }
  finally { action.value = '' }
}
async function refreshGeneration() {
  const chapter = activeChapter.value
  if (!chapter || action.value || selectionLoading.value) return
  action.value = 'refresh-generation'; error.value = ''
  try {
    const revisionResponse = await listContentRevisions(chapter.ID)
    if (activeChapter.value?.ID !== chapter.ID) return
    const authoritativeContentState = revisionResponse as typeof revisionResponse & { generation_run?: GenerationRun | null; generation_job?: GenerationJobView | null }
    generationRun.value = generationRunFromContentResponse(authoritativeContentState); generationJob.value = generationJobFromContentResponse(authoritativeContentState)
  } catch (e) { if (activeChapter.value?.ID === chapter.ID) error.value = e instanceof Error ? e.message : 'Không thể refresh generation state.' }
  finally { action.value = '' }
}
async function runRetryGeneration() {
  const chapter = activeChapter.value; const job = generationJob.value
  if (!chapter || !job || !mayRetryGeneration.value) return
  const requestChapterID = chapter.ID; const requestJobID = job.ID
  action.value = 'retry-generation'; error.value = ''
  try { const retried = await retryGenerationJob(requestJobID); if (activeChapter.value?.ID !== requestChapterID || generationJob.value?.ID !== requestJobID) return; generationJob.value = retried }
  catch (e) { if (activeChapter.value?.ID === requestChapterID && generationJob.value?.ID === requestJobID) error.value = e instanceof Error ? e.message : 'Không thể retry generation job.' }
  finally { if (activeChapter.value?.ID === requestChapterID) action.value = '' }
}
async function startNarration() {
  const chapter = activeChapter.value; const approved = approvedRevision.value
  if (!chapter || !approved || !mayCreateNarration.value) return
  const requestChapterID = chapter.ID; const requestRevisionID = approved.ID; const requestScript = approved.ContentText
  action.value = 'create-narration'; error.value = ''
  try {
    const created = await createNarrationRevision(requestChapterID, { source_content_revision_id: requestRevisionID, voice_id: preferredVoiceID.value, script: requestScript })
    if (activeChapter.value?.ID !== requestChapterID || approvedRevision.value?.ID !== requestRevisionID) return
    latestNarration.value = created
  } catch (e) { if (activeChapter.value?.ID === requestChapterID && approvedRevision.value?.ID === requestRevisionID) error.value = e instanceof Error ? e.message : 'Không thể tạo narration revision.' }
  finally { if (activeChapter.value?.ID === requestChapterID) action.value = '' }
}
async function runSynthesize() {
  const chapter = activeChapter.value; const narration = latestNarration.value
  if (!chapter || !narration || !maySynthesize.value) return
  action.value = 'synthesize'; error.value = ''; resetAudioPreview()
  try { const asset = await synthesizeNarration(chapter.ID, narration.ID); if (activeChapter.value?.ID !== chapter.ID) return; latestReadyAudio.value = asset; if (asset.IsActive) activeAudio.value = asset }
  catch (e) { if (activeChapter.value?.ID === chapter.ID) error.value = e instanceof Error ? e.message : 'Không thể synthesize narration thành READY audio.' }
  finally { action.value = '' }
}
async function runActivate() {
  const chapter = activeChapter.value; const ready = latestReadyAudio.value
  if (!chapter || !ready || !mayActivate.value) return
  action.value = 'activate'; error.value = ''; resetAudioPreview()
  try {
    const asset = await activateAudioAsset(chapter.ID, ready.ID)
    if (activeChapter.value?.ID !== chapter.ID) return
    activeAudio.value = asset; latestReadyAudio.value = asset.IsActive ? null : asset
    const narration = latestNarration.value; const refreshed = narration ? await getLatestReadyAudioAsset(chapter.ID, narration.ID) : null
  ...
  } catch (e) { if (activeChapter.value?.ID === chapter.ID) error.value = e instanceof Error ? e.message : 'Không thể activate READY audio asset.' }
  finally { if (activeChapter.value?.ID === chapter.ID) action.value = '' }
}
async function runMarkReady() {
  const chapter = activeChapter.value
  if (!chapter || !mayMarkReady.value) return
  const requestChapterID = chapter.ID; action.value = 'mark-ready'; error.value = ''
  try { const ready = await markChapterReady(requestChapterID); if (activeChapter.value?.ID !== requestChapterID) return; activeChapter.value = ready; chapters.value = chapters.value.map((item) => item.ID === ready.ID ? ready : item) }
  catch (e) { if (activeChapter.value?.ID === requestChapterID) error.value = e instanceof Error ? e.message : 'Không thể mark chapter ready.' }
  finally { if (activeChapter.value?.ID === requestChapterID) action.value = '' }
}
async function runPublish() {
  const chapter = activeChapter.value
  if (!chapter || !mayPublish.value) return
  const requestChapterID = chapter.ID; action.value = 'publish'; error.value = ''
  try { const published = await publishChapter(requestChapterID); if (activeChapter.value?.ID !== requestChapterID) return; activeChapter.value = published; chapters.value = chapters.value.map((item) => item.ID === published.ID ? published : item); publishReadiness.value = await getPublishReadiness(requestChapterID) }
  catch (e) { if (activeChapter.value?.ID === requestChapterID) error.value = e instanceof Error ? e.message : 'Không thể publish chapter.' }
  finally { if (activeChapter.value?.ID === requestChapterID) action.value = '' }
}
async function load() {
    ...
    <div v-if="!loading" class="workspace-grid">
      ...
      <main v-if="activeChapter" class="pipeline-panel" :aria-busy="selectionLoading">
        ...
        <section v-if="primaryStage" class="next-step" aria-labelledby="next-step-title">
          ...
          <div><span :class="['state-badge', `state-${primaryStage.state}`]">{{ stageStateLabel(primaryStage.state) }}</span><p>{{ primaryStage.summary }}</p><p v-if="primaryStage.blocker" class="blocker-copy">{{ primaryStage.blocker }}</p></div>
        </section>
        ...
        <section class="preview-panel" aria-labelledby="audio-preview-title">
          <div class="panel-heading">
            <div><p class="eyebrow">Authoritative preview</p><h3 id="audio-preview-title">Audio preview</h3></div>
            <span v-if="previewAsset" class="state-badge">READY v{{ previewAsset.VersionNo }}</span>
          </div>
          <p v-if="!previewAsset" class="empty-state">Preview khả dụng khi chapter có durable READY audio.</p>
          <template v-else>
            <p class="preview-copy">Nghe bản READY của chapter trước khi activate/publish. URL phát được cấp tạm thời qua admin boundary và không lộ storage key.</p>
            <button v-if="audioPreview.status === 'idle'" class="preview-button" type="button" :disabled="selectionLoading" @click="loadAudioPreview">Load audio preview</button>
            <p v-else-if="audioPreview.status === 'loading'" class="page-status" role="status">Đang cấp quyền preview audio…</p>
            <div v-else-if="audioPreview.status === 'error'" class="preview-error" role="alert">
        ...
        <section class="action-panel" aria-labelledby="actions-title">
          <div class="panel-heading"><div><p class="eyebrow">Safe actions</p><h3 id="actions-title">Production controls</h3></div><span v-if="action" class="action-progress" role="status">Đang xử lý…</span></div>
          <div class="actions">
            <button v-if="!generationRun" type="button" :disabled="!mayStartGeneration" @click="startGeneration">{{ action === 'start-generation' ? 'Đang bắt đầu…' : 'Start Generation' }}</button>
            <button v-if="generationRun || generationJob" type="button" :disabled="Boolean(action) || selectionLoading" @click="refreshGeneration">{{ action === 'refresh-generation' ? 'Đang refresh…' : 'Refresh Generation' }}</button>
            <button v-if="mayRetryGeneration" type="button" :disabled="!mayRetryGeneration" @click="runRetryGeneration">{{ action === 'retry-generation' ? 'Đang retry…' : 'Retry Generation' }}</button>
            <RouterLink class="action-link" :to="`/admin/stories/${storyID}/review`">Mở Content Review</RouterLink>
            <button type="button" :disabled="!mayCreateNarration" @click="startNarration">{{ action === 'create-narration' ? 'Đang tạo narration…' : 'Create Narration' }}</button>
            <button type="button" :disabled="!maySynthesize" @click="runSynthesize">{{ action === 'synthesize' ? 'Đang synthesize…' : 'Synthesize TTS' }}</button>
            <button type="button" :disabled="!mayActivate" @click="runActivate">{{ action === 'activate' ? 'Đang activate…' : 'Activate Audio' }}</button>
            <button type="button" :disabled="!mayMarkReady" @click="runMarkReady">{{ action === 'mark-ready' ? 'Đang mark ready…' : 'Mark Chapter Ready' }}</button>
            <button class="primary-action" type="button" :disabled="!mayPublish" @click="runPublish">{{ action === 'publish' ? 'Đang publish…' : 'Publish Chapter' }}</button>
          </div>
        </section>
        <details class="technical-details"><summary>Technical production details</summary><div class="detail-grid">
...
.next-step { display: grid; grid-template-columns: minmax(130px, .35fr) 1fr; gap: 18px; padding: 18px; margin-bottom: 18px; border: 1px solid var(--line); border-radius: 14px; background: var(--surface); }
...
.stage-card { min-width: 0; display: grid; grid-template-columns: 34px minmax(0, 1fr); gap: 12px; padding: 16px; border: 1px solid var(--line); border-radius: 14px; background: var(--surface); }
.stage-index { width: 32px; height: 32px; display: grid; place-items: center; border: 1px solid var(--line); border-radius: 50%; font-weight: 800; background: var(--surface-soft); }
...
@media (max-width: 1024px) { .workspace-grid { grid-template-columns: minmax(210px, 250px) minmax(0, 1fr); gap: 16px; }.stage-list { grid-template-columns: 1fr; }.next-step { grid-template-columns: 1fr; } }
@media (max-width: 760px) { .production-page { padding: 20px 16px 48px; }.production-header { flex-direction: column; margin-bottom: 18px; }.workspace-grid { grid-template-columns: 1fr; }.chapter-panel { position: static; padding: 14px; }.chapter-list { display: flex; gap: 8px; overflow-x: auto; padding-bottom: 4px; scroll-snap-type: x proximity; }.chapter-card { min-width: min(78vw, 280px); scroll-snap-align: start; }.pipeline-panel { padding: 16px; }.chapter-heading, .stage-title-row, .preview-panel .panel-heading { align-items: flex-start; }.actions { display: grid; grid-template-columns: 1fr; }.actions button, .action-link, .preview-button { width: 100%; justify-content: center; text-align: center; }.preview-button.secondary { justify-self: stretch; }.detail-grid p { grid-template-columns: 1fr; gap: 2px; } }
</style>
