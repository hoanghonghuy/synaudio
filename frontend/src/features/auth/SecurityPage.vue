...
async function requestDeletion() {
  if (
    !window.confirm(
      'Tài khoản sẽ bị vô hiệu hóa ngay lập tức. Bạn có chắc muốn bắt đầu yêu cầu xóa tài khoản không?',
    )
  ) {
    return
  }
  ...
  try {
    ...
    if (auth.user) {
      auth.user.status = 'DEACTIVATED'
    }
    success.value = 'Tài khoản đã được vô hiệu hóa. Bạn có thể hủy yêu cầu trong thời gian chờ.'
  } catch (e) {
...
if (!auth.initialized) {
  void auth.loadCurrentUser().then(() => {
    if (!auth.isAuthenticated) void router.replace('/auth')
  })
}
</script>
  ...
  <section class="page security-page">
    ...
    <div class="security-layout">
      <header class="security-heading">
        ...
        <p class="page-intro">
          Tăng thêm một lớp bảo vệ bằng ứng dụng xác thực. Synaudio không lưu mã xác thực của bạn ở trình duyệt.
        </p>
      </header>
      ...
      <section class="security-card" aria-labelledby="mfa-heading">
        <div class="section-heading">
          ...
          <span class="badge" :class="{ 'badge-active': mfaEnabled }">
            {{ mfaEnabled ? 'Đang bật' : 'Chưa bật' }}
          </span>
        </div>

        <p class="muted">
          Dùng Google Authenticator, 1Password hoặc ứng dụng TOTP tương thích để tạo mã gồm 6 chữ số.
        </p>

        <div v-if="!mfaEnabled && !secret" class="security-action">
          <p>Thiết lập MFA để bảo vệ các thao tác quản trị và tài khoản của bạn.</p>
          <button type="button" :disabled="loading" @click="beginSetup">
            {{ loading ? 'Đang chuẩn bị...' : 'Thiết lập MFA' }}
          </button>
        </div>
        ...
        <form v-if="secret" class="auth-form mfa-form" @submit.prevent="confirmSetup">
          ...
          <label for="mfa-code">
            ...
            <input
              ...
              required
            />
            <span id="mfa-code-help" class="field-help">Mã thay đổi sau mỗi khoảng thời gian ngắn.</span>
          </label>

          <p v-if="error" class="status-state error" role="alert">{{ error }}</p>
          <button type="submit" :disabled="loading || code.length !== 6">
            {{ loading ? 'Đang xác minh...' : 'Xác nhận và bật MFA' }}
          </button>
        </form>

        <div v-if="mfaEnabled" class="security-action">
          <p>MFA đang bảo vệ tài khoản của bạn. Hãy giữ ứng dụng xác thực hoạt động trước khi tắt tính năng này.</p>
          <button class="danger-button" type="button" :disabled="disabling" @click="disable">
            {{ disabling ? 'Đang tắt...' : 'Tắt MFA' }}
          </button>
        </div>
      ...
      <section v-if="hasRecoveryCodes" class="security-card recovery-card" aria-labelledby="recovery-heading">
        ...
        <p>
          Đây là lần duy nhất các mã này được hiển thị. Lưu chúng ở nơi an toàn; mỗi mã chỉ dùng được một lần.
        </p>
      ...
      <section class="security-card account-danger-zone" aria-labelledby="account-lifecycle-heading">
        ...
        <p>
          Yêu cầu này vô hiệu hóa tài khoản ngay lập tức. Dữ liệu cá nhân chỉ được ẩn danh sau thời gian chờ theo chính sách hệ thống.
        </p>
        <div v-if="accountDeactivated" class="status-state error" role="status">
          <strong>Tài khoản đang chờ xóa.</strong>
          <span>Bạn vẫn có thể hủy yêu cầu trong thời gian chờ.</span>
        </div>
        <div class="recovery-actions">
          <button
            v-if="accountDeactivated"
            class="secondary-button"
            type="button"
            :disabled="deleting"
            @click="cancelDeletion"
          >
            {{ deleting ? 'Đang xử lý...' : 'Hủy yêu cầu xóa' }}
          </button>
          <button v-else class="danger-button" type="button" :disabled="deleting" @click="requestDeletion">
