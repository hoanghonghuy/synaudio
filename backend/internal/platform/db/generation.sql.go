...
func (q *Queries) ListJobsByRun(ctx context.Context, runID pgtype.UUID) ([]GenerationJob, error) {
...
const listUsageByStory = `-- name: ListUsageByStory :many
...
func (q *Queries) ListUsageByStory(ctx context.Context, storyID pgtype.UUID) ([]GenerationJobAttempt, error) {
	rows, err := q.db.Query(ctx, listUsageByStory, storyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GenerationJobAttempt{}
	for rows.Next() {
		var i GenerationJobAttempt
		if err := rows.Scan(
			&i.ID,
...
const updateJobAttemptUsage = `-- name: UpdateJobAttemptUsage :one
UPDATE generation_job_attempts
SET usage = $2
WHERE id = $1
RETURNING id, job_id, attempt_no, provider, model, status, error_class, error_code,
...
type UpdateJobAttemptUsageParams struct {
	ID    pgtype.UUID `json:"id"`
	Usage []byte      `json:"usage"`
}

func (q *Queries) UpdateJobAttemptUsage(ctx context.Context, arg UpdateJobAttemptUsageParams) (GenerationJobAttempt, error) {
	row := q.db.QueryRow(ctx, updateJobAttemptUsage, arg.ID, arg.Usage)
	var i GenerationJobAttempt
	err := row.Scan(
		&i.ID,
		&i.JobID,
