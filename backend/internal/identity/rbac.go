...
func (s *AuthService) privilegedPrincipal(ctx context.Context, r *http.Request) (Principal, User, error) {
	principal, user, err := s.AuthenticateRequest(ctx, r)
	if err != nil {
		return Principal{}, User{}, err
	}
	if user.Status != StatusActive || user.EmailVerifiedAt == "" {
		if user.EmailVerifiedAt == "" {
			return principal, user, ErrEmailVerificationRequired
		}
	...
	if !assured {
		return principal, user, ErrMFARequired
	}
...
func (s *AuthService) ResolveAdmin(ctx context.Context, r *http.Request) (bool, error) {
	_, _, err := s.privilegedPrincipal(ctx, r)
	if err != nil {
		if errors.Is(err, ErrForbidden) || errors.Is(err, ErrMFARequired) || errors.Is(err, ErrEmailVerificationRequired) {
			return false, err
		}
		return false, err
	}
