...
// VerifyPrivilegedMFAChallenge binds MFA assurance to the exact authenticated
// session after a successful TOTP or single-use recovery-code verification.
// Stable error semantics allow downstream rate limiting (#50) without coupling.
func (s *AuthService) VerifyPrivilegedMFAChallenge(ctx context.Context, principal Principal, totpCode, recoveryCode string) error {
	if principal.UserID == "" || principal.SessionID == "" {
		return ErrUnauthenticated
	}
	method, err := s.store.GetMFAMethod(ctx, principal.UserID)
	if err != nil || method == nil || !method.Confirmed || method.Disabled {
