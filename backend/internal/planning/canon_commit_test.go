...
func (m *mockMemoryExtractor) ExtractMemory(_ context.Context, input MemoryExtractionInput) (MemoryExtraction, error) {
	m.input = input
...
func TestCommitCanonCreatesOfficialVersion(t *testing.T) {
	...
	svc := NewService(store, WithMemoryExtractor(extractor))
	...
	if res.Version.SourceContentRevisionID != "rev1" {
		t.Fatalf("expected source revision rev1, got %q", res.Version.SourceContentRevisionID)
	}
	if extractor.input.StoryID != "s1" || extractor.input.ChapterID != "ch1" || extractor.input.ContentRevisionID != "rev1" {
		t.Fatalf("unexpected memory extraction provenance: %+v", extractor.input)
	}
