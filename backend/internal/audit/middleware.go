...
// WrapRoute audits critical mutating routes. All admin mutations are covered,
// while non-admin writes are included only when explicitly listed in the
// semantic set; this avoids turning high-frequency listener progress into an
...
func eventForResponse(r *http.Request, method, route string, desc routeDescriptor, actorID string, status int, body []byte) Event {
	...
	for _, key := range []string{
		"generation_run_id", "plan_revision_id", "content_revision_id",
		"source_content_revision_id", "canon_version_id", "context_snapshot_id",
		"audio_asset_id", "narration_revision_id", "provider", "model",
	} {
		if value, ok := lookupReference(responseRefs, key); ok {
...
func auditDescriptor(method, route string) (routeDescriptor, bool) {
	key := strings.ToUpper(method) + " " + route
	if desc, ok := semanticRoutes[key]; ok {
		return desc, true
	}
	if strings.HasPrefix(route, "/admin/") {
		return routeDescriptor{Action: "ADMIN_MUTATION", ResourceType: "ADMIN_RESOURCE"}, true
	}
...
func firstString(values map[string]any, keys ...string) string {
	for _, key := range keys {
		if value, ok := lookupReference(values, key); ok {
...
func lookupReference(values map[string]any, target string) (any, bool) {
	normalizedTarget := normalizeJSONKey(target)
	return lookupReferenceValue(values, normalizedTarget)
}

func lookupReferenceValue(value any, normalizedTarget string) (any, bool) {
	switch typed := value.(type) {
	case map[string]any:
		for key, item := range typed {
			if normalizeJSONKey(key) == normalizedTarget {
				return item, true
			}
		}
		for _, item := range typed {
			if found, ok := lookupReferenceValue(item, normalizedTarget); ok {
				return found, true
			}
		}
	case []any:
